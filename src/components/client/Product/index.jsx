...
function DiscountCom() {
  ...
  if (token) {
   ...
   if (res.status===401) {
    ...
    axios.get("users/get_token/").then((res) => {
      const token = res.data.access_tokeSn;
  ...
  const addData = async (productaData) => {
    const formData = new FormData();
    formData.append("amount", 1);
    formData.append("product", productsData.id);
          ...
          {[...products].reverse()?.map((product) => {
            ...
            return (
              <div className="col-6 col-md-3 col-lg-3 mb-3" >
                <div className="h-100">
                  <div className=" flex flex-col text-gray-700 bg-white shadow-md bg-clip-border rounded-xl h-[350px] " key={product?.id}>
                    ...
                    <div className="p-2">
                      <div className="flex items-center justify-between mb-2">
                        <p className="block  text-blue-700 font-sans text-base antialiased font-medium leading-relaxed ">
                          {product?.title}
                        </p>
                      </div>
                      {product?.discount?.product_discount_price ? (
                        <div className="f">
                          <b className="text-xs text-red-900">{product?.discount?.product_discount_price?.toLocaleString("ru-Ru")} so'm</b>
                          <del>{product?.price.toLocaleString("ru-Ru")} so'm</del>
                        </div>
                      ) : (
                        <b className="text-xm text-red-900">{product?.price.toLocaleString("ru-Ru")} so'm</b>
                      )}
                    </div>
